...
<% loop Events %>
    <li class="vevent clearfix">
        <h3 class="summary"><a href="$Link"><% if Announcement %>$Title<% else %>$Event.Title<% end_if %></a></h3>
        ...
        <% else %>
            <% with Event %>
                ...
                $Content.FirstSentence(html)
                </div>
            <% end_with %>
            <a class="clearFix readMore" href="$Link">Read more></a>
        <% end_if %>
        <% if OtherDates %>
